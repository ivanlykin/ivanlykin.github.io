<?php
        ...
        if($from == "" || !preg_match ("/@/", $from) || $from == "E-mail") {
            $error_email = htmlspecialchars("Введите ваш E-mail");
            $error=true;
        }
        ...
        if(strlen($phone) == 0 || preg_match ("/[A-Za-z]/", $phone)) {
            $error_phone = "Введите ваш телефон";
            $error=true;
        }
        ...
        if($error==false){
            ...
            {
                $successMessage="Ваше резюме отправлено!"; 
            }
            else {
                $successMessage="Ошибка отправки сообщения! Попробуйте еще раз.";
            }
        }
